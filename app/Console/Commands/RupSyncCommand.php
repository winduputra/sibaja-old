...
class RupSyncCommand extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            if ($this->errors > 0) {
                throw new RuntimeException("Sinkronisasi RUP selesai dengan {$this->errors} kesalahan. Silakan coba lagi.");
            }
            ...
            return $this->reportDestructiveImportFailure($throwable, 'inaproc:sync-rup');
        }
    }
    ...
    {
        if ($type === 'all') {
            return [Satker::class, Penyedia::class, Swakelola::class, RupHistoryKajiUlang::class];
        }
        ...
        if ($type === 'penyedia') {
            return [Penyedia::class];
        }

        if ($type === 'swakelola') {
            return [Swakelola::class];
        }

        if ($type === 'history' || $type === 'history-kaji-ulang') {
            return [RupHistoryKajiUlang::class];
        }
